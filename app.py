...
async def predictRouteClient(request: Request):
    try:
        ...
        cost_df = shipping_data.get_input_data_frame()
        cost_predictor = model
        cost_df = preprocessor.transform(cost_df)

        cost_value = round(cost_predictor.predict(cost_df)[0], 2)
...
if __name__ == "__main__":
    model = pickle.load(open('shipping_price_model.pkl', 'rb'))
    preprocessor = pickle.load(open('shipping_preprocessor.pkl', 'rb'))
